...
foreach ($i in 2..11) {
    $classDir = "clase $i"
    $htmlFile = Join-Path $classDir "clase$i.html"
    $jsFile = Join-Path $classDir "script.js"

    if (Test-Path $htmlFile) {
        $html = [System.IO.File]::ReadAllText((Resolve-Path $htmlFile), $UTF8NoBOM)
        $html = $html -replace 'INGENIERÍA EN COMPUTACIÓN', 'INGENIERÍA EN SONIDO'
        $html = $html -replace 'Ingeniería en computación', 'Ingeniería en sonido'
        # Add "Ingeniería en sonido" as the subtitle for every class header
        $html = $html -replace 'Clase \d+: .*?</p>', "Clase ${i}: Ingeniería en sonido</p>"
        [System.IO.File]::WriteAllText((Resolve-Path $htmlFile), $html, $UTF8NoBOM)
    }

    if (Test-Path $jsFile) {
        $js = [System.IO.File]::ReadAllText((Resolve-Path $jsFile), $UTF8NoBOM)
        
        # 1. Replace VOCABULARY
        $vocabItems = $sonidoVocab[$i]
        $vocabJson = "["
        foreach ($item in $vocabItems) {
            $w = $item.w
            $m = $item.m
            $vocabJson += "`r`n    { word: '$w', meaning: '$m', category: 'default' },"
        }
        $vocabJson = $vocabJson.TrimEnd(',') + "`r`n];"
        
        $js = [regex]::Replace($js, 'const VOCABULARY = \[[\s\S]*?\];', "const VOCABULARY = $vocabJson")
        ...
        foreach ($item in $vocabItems) {
            ...
            if ($word.Length -gt 0) {
                $clue = $item.m
                # Alternate dir to make it slightly more interesting
                $dir = if ($y % 2 -eq 0) { 'down' } else { 'across' }
                $crosswordDataJson += "`r`n            { word: '$word', x: $x, y: $y, dir: '$dir', clue: '$clue' },"
                $x += 2
                $y += 2
            }
        }
        $crosswordDataJson = $crosswordDataJson.TrimEnd(',') + "`r`n        ];"
        
        $js = [regex]::Replace($js, 'const crosswordData = \[[\s\S]*?\];', "const crosswordData = $crosswordDataJson")

        [System.IO.File]::WriteAllText((Resolve-Path $jsFile), $js, $UTF8NoBOM)
    }
}
